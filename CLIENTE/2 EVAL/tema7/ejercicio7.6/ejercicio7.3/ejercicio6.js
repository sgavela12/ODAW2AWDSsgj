...
function cargarApiFetch() {
    // URL de la API
    const apiUrl = 'https://randomuser.me/api/?results=6&format=xml';

    // Configuración de la solicitud (puedes incluir el método, encabezados, cuerpo, etc.)
    const opciones = {
        method: 'GET',  // Método HTTP (GET, POST, etc.)
        headers: {
            'Content-Type': 'application/xml'  // Tipo de contenido esperado en la respuesta
            // Puedes agregar más encabezados si es necesario
        },
        // body: JSON.stringify({key: 'value'})  // Si estás haciendo una solicitud con cuerpo (por ejemplo, POST)
    };
    ...
    fetch(apiUrl, opciones)
        ...
        .catch(error => {
            // Manejar errores de red, del servidor o de cualquier otro tipo
