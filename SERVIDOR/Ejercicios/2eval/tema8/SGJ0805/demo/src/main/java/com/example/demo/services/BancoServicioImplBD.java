...
public class BancoServicioImplBD implements BancoServicio {
    ...
    public void borrarCuenta(String iban) {
        ...
        if (cuenta.getSaldo() != 0)
            throw new CuentaNoVaciaException();
    ...
    public Movimiento hacerMovimiento(String iban, Double importe) {
        Cuenta cuenta = cuentaRepositorio.findById(iban)
                .orElseThrow(() -> new CuentaNotFoundException(iban));
        ...
        return movimientoRepositorio.save(new Movimiento(null, cuenta, LocalDateTime.now(), importe));
    }

    public Cuenta convertDtoToCuenta(NuevaCuentaDto nuevaCuentaDto) {
        return new Cuenta(nuevaCuentaDto.getIban(), nuevaCuentaDto.getNombre(), 0.0, null);
    }

   

    public Movimiento convertDtoToMovimiento(NuevoMovimientoDto nuevoMovimientoDto) {
        return new Movimiento(null, cuentaRepositorio.findByIban(nuevoMovimientoDto.getIban()), LocalDateTime.now(),
